...
| cv | 재직·학위(대학원만, B.S. 는 CV 에만: user 09-21)·**Fellowships / Awards 분리**, PDF. 날짜·항목은 CV 와 대조됨 |
...
3. **설명 없는 전문 용어 금지.** `recall@K` 를 그대로 썼다가 user 가 무슨 뜻이냐고 물었다.
   물어봤다는 게 답이다. **09-21 두 번째:** 활동 달력 이름으로 `Commits` 를 권했다가 같은 질문을 받았다.
   공개 문구 후보를 내기 전에 "분야 밖 방문자가 설명 없이 아는 말인가" 를 먼저 거를 것.
